...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="First year to process (2015)."
    )
...
def main(logger):
    ...
    csv_path = "/leonardo_work/EUHPC_D27_095/kkingston/IPSL-AID/data/dates_hours_1980_2022.csv"
    ...
    for year in range(year_start, year_end + 1):
        ...
        logger.start_task(
            "Processing year",
            description="Generating ERA5 samples",
            year=year
        )
        ...
        grouped_month = df_year.groupby(
            [df_year["datetime"].dt.year,
             df_year["datetime"].dt.month]
        )
        ...
        for variable in variables:
            ...
            variable_root = os.path.join(
                base_data_root,
                f"data_{variable.upper()}"
            )
            
            # Output directory for this variable
            output_root = os.path.join(
                base_output_root,
                f"data_FOURxDaily_{variable.upper()}"
            )
