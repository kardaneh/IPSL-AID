...
class SongUNet(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if isinstance(img_resolution, (tuple, list)):
            assert len(img_resolution) == 2, "img_resolution must be a tuple/list (height, width)"
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                cin = cout
                cout = model_channels
                self.enc[f'{res_key}_conv'] = Conv2d(in_channels=cin, out_channels=cout, kernel=3, **init)
            else:
                self.enc[f'{res_key}_down'] = UNetBlock(in_channels=cout, out_channels=cout, down=True, **block_kwargs)
                if encoder_type == 'skip':
                    self.enc[f'{res_key}_aux_down'] = Conv2d(in_channels=caux, out_channels=caux, kernel=0, down=True, resample_filter=resample_filter)
                    self.enc[f'{res_key}_aux_skip'] = Conv2d(in_channels=caux, out_channels=cout, kernel=1, **init)
                if encoder_type == 'residual':
                    self.enc[f'{res_key}_aux_residual'] = Conv2d(in_channels=caux, out_channels=cout, kernel=3, down=True, resample_filter=resample_filter, fused_resample=True, **init)
                    caux = cout
            for idx in range(num_blocks):
                ...
                self.enc[f'{res_key}_block{idx}'] = UNetBlock(in_channels=cin, out_channels=cout, attention=attn, **block_kwargs)
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            if level == len(channel_mult) - 1:
                self.dec[f'{res_key}_in0'] = UNetBlock(in_channels=cout, out_channels=cout, attention=True, **block_kwargs)
                self.dec[f'{res_key}_in1'] = UNetBlock(in_channels=cout, out_channels=cout, **block_kwargs)
            else:
                self.dec[f'{res_key}_up'] = UNetBlock(in_channels=cout, out_channels=cout, up=True, **block_kwargs)
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
                # Check attention for rectangular resolution
                attn = (idx == num_blocks and (min(res_h, res_w) in attn_resolutions))
                self.dec[f'{res_key}_block{idx}'] = UNetBlock(in_channels=cin, out_channels=cout, attention=attn, **block_kwargs)
            if decoder_type == 'skip' or level == 0:
                if decoder_type == 'skip' and level < len(channel_mult) - 1:
                    self.dec[f'{res_key}_aux_up'] = Conv2d(in_channels=out_channels, out_channels=out_channels, kernel=0, up=True, resample_filter=resample_filter)
                self.dec[f'{res_key}_aux_norm'] = GroupNorm(num_channels=cout, eps=1e-6)
                self.dec[f'{res_key}_aux_conv'] = Conv2d(in_channels=cout, out_channels=out_channels, kernel=3, **init_zero)
...
class DhariwalUNet(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # Handle rectangular resolution
        if isinstance(img_resolution, (tuple, list)):
            assert len(img_resolution) == 2, "img_resolution must be a tuple (height, width)"
            self.img_resolution = img_resolution
            self.img_height, self.img_width = img_resolution
        else:
            self.img_resolution = (img_resolution, img_resolution)
        ...
        # Mapping.
        self.map_noise = PositionalEmbedding(num_channels=model_channels) if diffusion_model else None
    ...
    **model_kwargs : dict
    ...
    def __init__(self,
        ...
        **model_kwargs : dict
        ...
        self.model = globals()[model_type](img_resolution=img_resolution, in_channels=in_channels, out_channels=self.out_channels, label_dim=label_dim, **model_kwargs)
    ...
    def forward(self, x, sigma, condition_img=None, class_labels=None,
        ...
        **model_kwargs : dict
        ...
        """
        in_img = torch.cat([x, condition_img], dim=1) if condition_img is not None else x
    ...
    def forward(self, x, sigma, condition_img=None, class_labels=None,
        ...
        **model_kwargs : dict
        ...
        if self.label_dim == 0:
            ...
            class_labels = torch.zeros([in_img.shape[0], self.label_dim], device=in_img.device)
        else:
            class_labels = class_labels.to(torch.float32).reshape(-1, self.label_dim)
...
class TestDiffusionNetworks(unittest.TestCase):
    ...
    def test_song_unet_square_resolution(self):
        ...
        x = torch.randn(self.batch_size, self.in_channels, img_resolution, img_resolution).to(self.device)
        cond_img = torch.randn(self.batch_size, self.cond_channels, img_resolution, img_resolution).to(self.device)
        input_img = torch.cat([x, cond_img], dim=1)  # Concatenate along channel dimension
        
        noise_labels = torch.randn(self.batch_size).to(self.device)
        class_labels = torch.randint(0, self.label_dim, (self.batch_size,)).to(self.device)
        
        with torch.no_grad():
            output = model(input_img, noise_labels, class_labels)
        
        self.assertEqual(output.shape, (self.batch_size, self.out_channels, img_resolution, img_resolution))
        if self.logger:
            self.logger.info(f"✅ SongUNet square test passed - output shape: {output.shape}")
    ...
    def test_song_unet_rectangular_resolution(self):
        ...
        # Test forward pass - concatenate input and conditional image
        x = torch.randn(self.batch_size, self.in_channels, *img_resolution).to(self.device)
        cond_img = torch.randn(self.batch_size, self.cond_channels, *img_resolution).to(self.device)
        input_img = torch.cat([x, cond_img], dim=1)  # Concatenate along channel dimension
        ...
        self.assertEqual(output.shape, (self.batch_size, self.out_channels, *img_resolution))
        if self.logger:
            self.logger.info(f"✅ SongUNet rectangular test passed - output shape: {output.shape}")
    ...
    def test_dhariwal_unet(self):
        ...
        noise_labels = torch.randn(self.batch_size).to(self.device)
        class_labels = torch.randint(0, self.label_dim, (self.batch_size,)).to(self.device).float()  # Convert to float
        ...
        self.assertEqual(output.shape, (self.batch_size, self.out_channels, *img_resolution))
        if self.logger:
            self.logger.info(f"✅ DhariwalUNet test passed - output shape: {output.shape}")
    ...
    def test_vp_preconditioner(self):
        ...
        class_labels = torch.randint(0, self.label_dim, (self.batch_size, 2), device=self.device)
    ...
    def test_edm_preconditioner(self):
        ...
        self.assertEqual(output.shape, x.shape)
        if self.logger:
            self.logger.info(f"✅ EDMPrecond test passed - output shape: {output.shape}")
    ...
    def test_parameter_counts(self):
        ...
        configs = [
            ('SongUNet-Small', SongUNet, {'model_channels': 32, 'channel_mult': [1, 2]}),
            ('SongUNet-Medium', SongUNet, {'model_channels': 64, 'channel_mult': [1, 2, 2]}),
            ('DhariwalUNet-Small', DhariwalUNet, {'model_channels': 32, 'channel_mult': [1, 2]})
        ]
        ...
        for name, model_class, kwargs in configs:
            with self.subTest(model=name):
                model = model_class(
                    img_resolution=64,
                    in_channels=self.in_channels + self.cond_channels,  # Use total channels
                    out_channels=self.out_channels,
                    label_dim=self.label_dim,
                    **kwargs
                ).to(self.device)
                ...
                self.assertGreater(total_params, 1000)  # Should have at least 1K parameters
                if self.logger:
