...
output_dir = "../data_Rosie/"  # Output directory for processed samples
...
for day in unique_days:
    # Find all indices corresponding to this specific day
    day_inds = np.where(times.date == day)[0]
    day_times = times[day_inds]
    ...
    if len(selected) == 4:
        ...
        print(f"Not all fixed hours available for {day}, found {len(selected)}")
